...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"seg.totalCost": "总费",
			"cfg.lang": "语言",
			"card.title": "DeepSeek 用量监控（dsh-deepseek-monitor）",
			"card.desc": "会话头部 / 侧边栏的 DeepSeek 平台用量面板配置",
			"config.desc.reorder": "顺序（≡ 拖拽）请在会话头部 ⚙ 面板调整",
		...
		const en = {
			...
			"cfg.lang": "Language",
			"card.title": "DeepSeek usage monitor (dsh-deepseek-monitor)",
			"card.desc": "Config for the DeepSeek usage panels in the session header / sidebar",
			"config.desc.reorder": "Order (drag ≡) is adjusted in the ⚙ panel of the session header"
		};
		//#endregion
		...
		//#region 4) 设置 → 插件配置卡片（自包含，复用 /dsm/config 与共享配置）
		function DsmUsageSettingsCard() {
			const cfg = useCfg();
			const tr = (key) => trKey(cfg.lang, key);
			...
			return react.createElement(
				"div",
				{ style: { display: "flex", flexDirection: "column", gap: "2px", padding: "4px 0" } },
				react.createElement("div", { style: S.configTitle, children: tr("card.title") }),
				react.createElement("div", { style: S.configDesc, children: tr("card.desc") }),
				react.createElement(
					"div",
					{ style: { display: "flex", alignItems: "center", gap: "6px", margin: "8px 0 2px" } },
					react.createElement(
						"span",
						{ style: { font: F.small, color: "var(--dsw-alias-label-secondary, #999)", marginRight: "auto" } },
						tr("cfg.lang")
				...
				react.createElement(
					"div",
					{ style: { font: F.tiny, color: "var(--dsw-alias-label-tertiary, #888)", marginTop: "6px" } },
					tr("config.desc.reorder")
				)
			);
		}
		//#endregion
		...
		/**
		 * 客户端插件体：注册字典 + 展示位（会话头部 + 侧边栏 + 设置页插件配置卡片；
		...
		function apply(ctx) {
			...
			// 3) 设置 → 插件配置：插件卡片（自包含，复用 /dsm/config）
			ctx.slots.inject(
