...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"cfg.lowBalance.threshold": "提醒阈值（¥）",
			"cfg.lowBalance.interval": "提醒间隔（小时）",
			"alert.title": "余额不足",
			"alert.balance": "当前余额",
		...
		const en = {
			...
			"cfg.lowBalance.interval": "Remind every (hours)",
		...
		function DsmUsageAction(props) {
			...
			const [press, setPress] = react.useState(null); // {key,x,y} | null
			const [drag, setDrag] = react.useState(null); // {key,target} | null
			const dragTimer = react.useRef(null);
			const dragStart = react.useRef(null);
			const dragged = react.useRef(false); // 本次按下是否已变成拖动（用于吞掉随后的那次 click）
			const segRefs = react.useRef(new Map());
			// 余额不足提醒（弹窗开关状态）
			...
			/** 当前可见的键（顺序即显示顺序）。 */
			const visibleKeys = () => (cfg.order || ITEM_KEYS).filter((k) => !!cfg[k]);
			...
			/** 按下：到长按时长才真的进入拖动。 */
			const onSegPointerDown = (key) => (e) => {
				if (drag || e.button !== 0) return;
				dragged.current = false;
				dragStart.current = { x: e.clientX, y: e.clientY };
				setPress({ key, x: e.clientX, y: e.clientY });
				dragTimer.current = setTimeout(() => {
					dragTimer.current = null;
					dragged.current = true;
					setOpen(null); // 拖动时收起弹出面板，免得挡住
					setDrag({ key, target: visibleKeys().indexOf(key) });
				}, LONG_PRESS_MS);
			};

			/** 拖动中：按其它块的中心点算目标位次；顺序即时重排＝预览。 */
			const onDragPointerMove = (e) => {
				const st = dragStart.current;
				if (dragTimer.current !== null && st) {
					const far =
				...
				for (const k of visibleKeys()) {
					...
					const el = segRefs.current.get(k);
					if (!el) continue;
			...
			const endDrag = () => {
				...
				setDrag(null);
				if (!d) return;
				const cur = visibleKeys().indexOf(d.key);
				if (cur < 0 || cur === d.target) return;
				updateCfg("order", moveInOrder(cfg.order || ITEM_KEYS, (k) => !!cfg[k], d.key, d.target));
			};

			// 拖动期间监听挂在 window 上：块会随顺序重排（DOM 节点移动），
			// 挂在块自己身上会丢事件。capture 阶段接管，避免被宿主的手势吃掉。
			react.useEffect(() => {
			...
			const renderOrder = drag
				? moveInOrder(cfg.order || ITEM_KEYS, (k) => !!cfg[k], drag.key, drag.target)
				: null;
			const segments = buildSegments(tr, cfg, data, renderOrder);

			const headerRow = react.createElement(
				"div",
				{ style: drag ? { ...S.row, cursor: "grabbing" } : S.row },
				segments.map((s) =>
					react.createElement(Segment, {
						...
						dot: s.dot,
						tooltip: s.tooltip,
						dragging: !!drag && drag.key === s.key,
						segRef: (el) => {
							if (el) segRefs.current.set(s.key, el);
							else segRefs.current.delete(s.key);
						},
						onPointerDown: onSegPointerDown(s.key),
						onClick: () => {
							if (dragged.current) {
								dragged.current = false; // 长按拖过之后的那次 click 不再打开面板
								return;
							}
							togglePanel("detail");
						}
			...
			return renderRoot(
				react.createElement(
					...
					!!cfg.lowBalanceAlert &&
						react.createElement(
							"div",
							{ style: { display: "flex", flexWrap: "wrap", gap: "10px", padding: "0 0 6px" } },
