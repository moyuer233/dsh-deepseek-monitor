...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"cfg.sidebar.items.hint": "侧边栏用自己的一套，与上面头部的顺序无关：开关控制显示，↑↓ 调顺序；也可以直接在侧边栏里长按某一项拖动排序。",
			"cfg.sidebar.up": "上移",
			"cfg.sidebar.down": "下移",
		...
		const en = {
			...
			"cfg.sidebar.items.hint": "The sidebar keeps its own set, independent of the header above: switches control visibility, ↑/↓ change the order — or long-press a row in the sidebar and drag it.",
			"cfg.sidebar.up": "Move up",
			"cfg.sidebar.down": "Move down",
		...
		function SideRow(props) {
			return react.createElement(
				...
				react.createElement(
					...
					{
						type: "button",
						style: { ...S.miniBtn, opacity: props.canUp ? 1 : 0.3 },
				...
				react.createElement(
					...
					{
						type: "button",
						style: { ...S.miniBtn, opacity: props.canDown ? 1 : 0.3 },
		...
		function DsmUsageAction(props) {
			...
			return renderRoot(
				react.createElement(
					...
					// 侧边栏显示项：与头部解耦的独立一套（开关 + ↑↓；侧边栏里也能长按拖）
					!!cfg.sidebar &&
						(() => {
							const order = sidebarOrderOf(cfg);
							const shown = sidebarItemsOf(cfg);
							const vis = order.filter((k) => shown.includes(k));
							const move = (key, delta) => {
								const cur = vis.indexOf(key);
								const target = cur + delta;
								if (cur < 0 || target < 0 || target >= vis.length) return;
								updateCfg("sidebarOrder", moveInOrder(order, (k) => shown.includes(k), key, target));
							};
							...
							return react.createElement(
								...
								order.map((key) =>
									react.createElement(SideRow, {
										...
										canUp: vis.indexOf(key) > 0,
										canDown: vis.indexOf(key) >= 0 && vis.indexOf(key) < vis.length - 1,
										upTitle: tr("cfg.sidebar.up"),
										downTitle: tr("cfg.sidebar.down"),
										onUp: () => move(key, -1),
										onDown: () => move(key, +1),
