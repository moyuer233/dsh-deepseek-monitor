...
- **侧边栏独立配置**：侧边栏有自己的一套「显示哪些项 + 顺序」，与头部互不影响；面板里用开关 + ↑↓ 调整，也可以直接在侧边栏里长按某一行拖动排序
...
- 侧边栏是**独立的一套**：面板「侧边栏显示项」里用开关 + ↑↓ 调整，或直接在侧边栏里长按拖动；改动只影响侧边栏，不动头部
...
- 侧边栏改为**独立配置**：显示哪些项与顺序都与头部解耦（面板里开关 + ↑↓，也可在侧边栏里长按拖动）；老配置自动按头部推导，升级后外观不变
